...
class Predictor(nn.Module):
    def __init__(
        ...
        self.mlp_layers = nn.ModuleList(
            [MultiLayerPerceptron(hidden_dim, hidden_dim, dropout) for _ in range(num_layers)]
        )
        self.output_proj = nn.Linear(in_steps * hidden_dim, out_steps * out_dim)
    ...
    def forward(self, x):
        ...
        out = out.reshape(
            batch_size, num_nodes, in_steps * self.hidden_dim
        )
        out = self.output_proj(out).view(
            batch_size, num_nodes, self.out_steps, self.out_dim
        )
...
class DMRCMLP(nn.Module):
    def __init__(
        ...
        self.x_predictor = Predictor(
            self.model_dim, self.model_dim, output_dim, in_steps, out_steps, num_branch_layers, dropout)
