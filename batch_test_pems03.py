...
for seed in seeds:
    config.set("seed", seed)
    # with reconstruction, and with fixed masking (M2)
    config.config["model_args"]["use_recon"] = True
...
for seed in seeds:
    config.set("seed", seed)
    # without reconstruction, but with diminishing masking (M3)
...
for seed in seeds:
    config.set("seed", seed)
    # without reconstruction, and without masking (M4)
    config.config["model_args"]["use_recon"] = False
    config.set("adaptive_mask", False)
